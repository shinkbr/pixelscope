...
function buildPngBytes(trailing: number[] = []): Uint8Array {
  return new Uint8Array([
    ...
    0x00,
    0x00,
    0x00, // bit depth/color/interlace
    0x00,
    0x00,
    0x00,
    0x00, // IHDR crc (not validated by parser)
    0x00,
    0x00,
    0x00,
    0x00, // IEND length
...
function buildJpegBytes(trailing: number[] = []): Uint8Array {
  return new Uint8Array([
    ...
    0x04,
    0x11,
    0x22, // APP0 segment with 2-byte payload
    0xff,
    0xd9, // EOI
    ...trailing,
  ]);
...
test("extractTrailingData returns null for truncated PNG chunk", () => {
  const source = new Uint8Array([
    ...
    0x00,
    0x00,
    0x20, // too large for provided bytes
    0x49,
    0x48,
    0x44,
    0x52,
...
test("extractTrailingData handles JPEG with stuffed marker bytes", () => {
  const source = new Uint8Array([
    ...
    0xd8, // SOI
    0xff,
    0x00, // stuffed 0xFF byte in entropy stream
    0xff,
    0xd9, // EOI
    0xaa,
  ]);
...
test("extractTrailingData returns null for JPEG invalid segment lengths", () => {
  ...
  const missingLengthBytes = new Uint8Array([
    ...
    0xff,
    0xe2,
    0x00, // marker but incomplete length
  ]);
  const segmentOverrun = new Uint8Array([
    ...
    0x00,
    0x08,
    0x01, // segment too short for declared length
  ]);
